...
function InputField({ placeHolder, onSubmit}) {
    ...
    return (
        <form className="relative w-full">
            ...
            <input
                type="text"
                className="h-24 py-3 px-32 block w-full focus:outline-none border-2 placeholder-grey border-white rounded-full text-2xl focus:border-grey  bg-light-grey"
                placeholder={placeHolder}
                onChange={(e) => setInputWord(e.target.value)}
                onKeyDown={(e) => { if (e.key === 'Enter') onSubmit(inputWord) }}
                />
            <div className="absolute inset-y-0 right-0 flex items-center pr-3">
                <button type='submit' onClick={() => onSubmit(inputWord)} className='w-16 h-16s bg-purple rounded-full'>
                    <svg viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg" stroke="#ffffff"><g id="SVGRepo_bgCarrier" stroke-width="0"></g><g id="SVGRepo_tracerCarrier" stroke-linecap="round" stroke-linejoin="round" stroke="#CCCCCC" stroke-width="0.048"></g><g id="SVGRepo_iconCarrier"> <path d="M5 12H19M19 12L13 6M19 12L13 18" stroke="#ffffff" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"></path> </g></svg>
                </button>
            </div>
        </form>
    )
